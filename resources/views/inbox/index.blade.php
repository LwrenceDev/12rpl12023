...
<div class="product-card shadow">
    <a class="btn btn-primary mb-3 mt-3" href="/inbox/create">
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{Carbon\Carbon::create($x->tanggal_terima)->format('d F Y')}}</td>
          <td>{{$x->no_surat}}</td>
          <td>{{$x->pengirim}}</td>
          <td class="text-center">
            <a class="btn badge bg-success" href="/inbox/{{$x->id}}">
            ...
            <a class="btn badge bg-warning" href="/inbox/{{$x->id}}/edit">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
              ...
              Edit</a>
           @if ($x->relasi==1)
              <button disabled type="submit" class="btn badge bg-danger">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            ...
            @else
            <form onclick="return confirm('apakah kamu yakin hapus data?')" action="/inbox/{{$x->id}}" method="post"
              class="d-inline">
              @csrf
              @method('delete')
